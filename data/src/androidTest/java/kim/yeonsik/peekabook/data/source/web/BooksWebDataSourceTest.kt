...
class BooksWebDataSourceTest {
    private val books = BookRepositoryImpl(MockDataSource())

    @Test
    fun testGetByTitle() {
        val actual = books.get("초보자", 1)
        assertEquals(1, actual.size)
    }

    @Test
    fun testGetByIsbn() {
        val actual = books.get("8956747814")
        assertEquals("8956747814", actual.isbn[0])
    }
}


class MockDataSource: RemoteBookDataSource {
    ...
    override fun get(title: String, page: Int): List<kim.yeonsik.peekabook.domain.model.Book> {
        return response.mapToBook().filter { it.title.contains(title) }
    }
