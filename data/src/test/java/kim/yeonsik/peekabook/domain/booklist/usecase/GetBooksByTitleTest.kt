...
class GetBooksByTitleTest: AutoCloseKoinTest() {
    val repositoryModule = module {
        single<kim.yeonsik.peekabook.domain.repository.BookRepository> { MockRepositoryImpl() }
    }
    ...
    fun testGetBooksByTitle() {
        kim.yeonsik.peekabook.domain.booklist.usecase.GetBooksByTitle(MockRepositoryImpl())
            .get("", 1, object : kim.yeonsik.peekabook.domain.UseCase.Callback<List<kim.yeonsik.peekabook.domain.model.Book>>{
            override fun onSuccess(result: List<kim.yeonsik.peekabook.domain.model.Book>, nextPage: Int) {
...
class MockRepositoryImpl: kim.yeonsik.peekabook.domain.repository.BookRepository {
    ...
    override fun get(title: String, page: Int): List<kim.yeonsik.peekabook.domain.model.Book> {
        return response.mapToBook()
    }

    override fun get(isbn: String): kim.yeonsik.peekabook.domain.model.Book {
        return response.mapToBook().first { it.isbn.contains(isbn) }
    }

    private fun BookResponse.mapToBook(): List<kim.yeonsik.peekabook.domain.model.Book> {
        return this.documents.map { it.toBook() }
    }

    private fun Document.toBook(): kim.yeonsik.peekabook.domain.model.Book {
        return kim.yeonsik.peekabook.domain.model.Book(
            authors,
            contents,
            datetime,
            isbn.split(" "),
